...
const getPie3D = (pieData: PieData[], color: string[], internalDiameterRatio: number, distance: number, alpha: number, pieHeight: number, opacity = 1) => {
    const series = []
    let sumValue = 0
    let startValue = 0
    let endValue = 0
...
/**
   * 获取3d丙图的最高扇区的高度
   */
const getHeight3D = (series: Record<string, any>[], height: number) => {
    series.sort((a, b) => {
